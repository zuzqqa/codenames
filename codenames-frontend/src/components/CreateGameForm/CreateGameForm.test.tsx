...
describe('CreateGameForm', () => {
  ...
  it('updates game name when user types', async () => {
    const user = userEvent.setup();
    render(<CreateGameForm soundFXVolume={50} />);

    const input = screen.getByPlaceholderText('game-name') as HTMLInputElement;
  ...
  it('changes deck language', async () => {
    const user = userEvent.setup();
    render(<CreateGameForm soundFXVolume={50} />);
  ...
  it('toggles voice chat checkbox', async () => {
    const user = userEvent.setup();
    render(<CreateGameForm soundFXVolume={50} />);

    const checkboxes = screen.getAllByAltText('Checkmark');
    const voiceChatCheckbox = checkboxes[1];

    await user.click(voiceChatCheckbox);

    await waitFor(() => {
      const updatedCheckboxes = screen.getAllByAltText('Checkmark');
  ...
  it('shows error when game name is empty', async () => {
    ...
    const submitButton = screen.getByRole('button', { name: /create-game-button/i });
  ...
  it('shows error when private lobby has no password', async () => {
    ...
    await waitFor(() => {
      expect(mockAddToast).toHaveBeenCalledWith('private-lobby-password-error', 'error');
    });
  });
  ...
  it('shows error when API request fails', async () => {
    ...
    await waitFor(() => {
      expect(mockAddToast).toHaveBeenCalledWith('Failed to create game session', 'error');
    });
  });
