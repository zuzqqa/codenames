...
const Gameplay: React.FC<GameplayProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      (redTeamPlayers.length < 2 ||
        ...
        redTeamPlayers.length + blueTeamPlayers.length < 4) &&
  ...
  useEffect(() => {
    ...
    gameSession,
    gameSession?.gameState.redTeamLeader,
    gameSession?.gameState.blueTeamLeader,
    hasPlayerDisconnected,
  ]);
  ...
  useEffect(() => {
    if (redTeamScore >= 9 || blueTeamScore >= 8) {
      revealCardsVotedByTeam();
      let winner = redTeamScore >= blueTeamScore ? "red" : "blue";
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        !target.closest(".card-black-img") &&
        !target.closest(".codename-input-container")
      ) {
        setIsCardVisible(false);
        setCardText("");
        setCardNumber(1);
  ...
  const disconnectUser = () => {
    ...
    fetch(
      ...
      .catch((error) => {
        addToast(
          "An error occurred while disconnecting: " + error.message,
          "error"
        );
      });
  };
            ...
            <div className="item codename-item">
              ...
              >
                <div className="codename-card-text">
                  <span>{gameSession?.gameState.hint || "HINT"}</span>
                  <span>
                    {gameSession?.gameState.hintNumber === "0"
                      ? ""
